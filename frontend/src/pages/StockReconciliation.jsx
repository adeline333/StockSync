      ...
      <main className="flex-1 flex flex-col min-h-screen">
        <header className="h-20 bg-white border-b border-slate-200 px-8 flex items-center justify-between sticky top-0 z-10">
          <div>
            ...
            <p className="text-sm text-slate-500">Sales vs. Inventory Matcher</p>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={exportCSV} disabled={!items.length}
              className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors disabled:opacity-40">
        ...
        <div className="p-8 flex-1 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[160px]">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Period Start</label>
              <div className="relative">
                <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input type="date" value={periodStart} onChange={e => setPeriodStart(e.target.value)}
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-sky-500" />
              </div>
            </div>
            <div className="flex-1 min-w-[160px]">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Period End</label>
              <div className="relative">
                <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            ...
            <div className="flex-1 min-w-[160px]">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Location</label>
              <div className="relative">
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-5">
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 relative overflow-hidden">
                  ...
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Reconciliation Score</p>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 relative overflow-hidden">
                  ...
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Unmatched Records</p>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 relative overflow-hidden">
                  ...
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Variance Value</p>
              ...
              <div className="flex gap-6">
                ...
                <div className="flex-[2] bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                  <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <h2 className="text-base font-bold text-slate-800">Transaction Match — {items.length} items</h2>
                  </div>
                  <div className="grid grid-cols-[2fr_1fr_1fr_1fr] bg-slate-50 px-6 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-100">
                    ...
                    {items.length === 0 ? (
                      ...
                      return (
                        <div key={item.id} onClick={() => setSelectedItem(item.status !== 'matched' ? item : null)}
                          ...
                          <div className="text-sm font-semibold text-slate-700">{item.sales_qty}</div>
                ...
                {/* Discrepancy Panel */}
                <div className="flex-1 bg-white rounded-2xl shadow-lg border border-slate-100 flex flex-col overflow-hidden min-w-[300px]">
                  ...
                  {!selectedItem ? (
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-5">
                      <div>
                        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Issue Summary</p>
                      ...
                      <div className="border-t border-slate-100 pt-4">
                        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Resolution Actions</p>
                        <div className="space-y-2">
                          ...
                          <button onClick={() => navigate(`/reconciliation/${recon.id}?item=${selectedItem.id}`)}
                            className="w-full bg-white border border-slate-200 text-slate-600 rounded-xl py-3 text-sm font-bold hover:bg-slate-50 transition-colors">
