    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex font-sans overflow-hidden">
      ...
      <main className="flex-1 ml-[100px] flex flex-col h-screen overflow-hidden">
        <header className="h-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-8 flex items-center justify-between shrink-0">
          <button onClick={() => navigate('/pos')} className="flex items-center text-slate-500 dark:text-slate-400 hover:text-sky-600 font-bold group">
            <ArrowLeft className="w-5 h-5 mr-2 group-hover:-translate-x-1 transition-transform" /> Back to Sale
          </button>
          <h1 className="text-2xl font-black text-slate-800 dark:text-slate-100">Order Checkout</h1>
          <span className="text-sm font-bold text-slate-400 uppercase tracking-wider">Review Order</span>
        </header>

        <div className="flex-1 p-8 overflow-y-auto bg-slate-50 dark:bg-slate-950 flex gap-8 max-w-[1400px] mx-auto w-full">
          {/* Left: Items */}
          <div className="flex-[3] bg-white p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col">
            <div className="grid grid-cols-[3fr_1fr_1fr_1fr] border-b border-slate-100 dark:border-slate-800 pb-4 mb-4 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
            ...
            <div className="space-y-3 flex-1 overflow-y-auto">
              {cart.map(item => (
                <div key={item.id} className="grid grid-cols-[3fr_1fr_1fr_1fr] items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 dark:border-slate-800 group hover:border-sky-200 transition-all">
                  <div className="flex items-center">
                    <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center mr-4 border border-slate-200 dark:border-slate-700 shrink-0">
                  ...
                  <div className="text-sm font-bold text-slate-600 dark:text-slate-400 pl-4">{item.price.toLocaleString()}</div>
                  <div className="flex justify-center">
                    <div className="w-10 h-10 rounded-lg bg-white border border-slate-200 dark:border-slate-700 flex items-center justify-center font-black text-slate-800 dark:text-slate-100 shadow-sm">{item.qty}</div>
                  </div>
                  <div className="flex items-center justify-end gap-3">
                    <span className="text-base font-black text-slate-800 dark:text-slate-100">{(item.price * item.qty).toLocaleString()}</span>
            ...
            <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 block flex items-center gap-2">
                <FileText className="w-4 h-4 text-slate-400" /> Order Notes
              </label>
              <textarea rows={3} value={notes} onChange={e => setNotes(e.target.value)}
                placeholder="Add any special instructions..."
                className="w-full bg-slate-50 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 text-sm font-medium text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:bg-white transition-all resize-none" />
            </div>
          ...
          {/* Right: Summary */}
          <div className="flex-[2] bg-white p-8 rounded-3xl shadow-lg border border-slate-100 dark:border-slate-800 flex flex-col justify-between">
            <div>
              <h2 className="text-2xl font-black text-slate-800 dark:text-slate-100 mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">Payment Summary</h2>
              <div className="space-y-4">
                <div className="flex justify-between bg-slate-50 px-4 py-3 rounded-xl border border-slate-100 dark:border-slate-800">
                  <span className="text-slate-500 dark:text-slate-400 text-sm font-bold">Subtotal</span>
                ...
                <div className="flex justify-between bg-slate-50 px-4 py-3 rounded-xl border border-slate-100 dark:border-slate-800">
                  <span className="text-slate-500 dark:text-slate-400 text-sm font-bold">VAT (18%)</span>
              ...
              <div className="flex justify-between items-end mb-8">
                <span className="text-slate-800 dark:text-slate-100 font-black text-xl">Grand Total</span>
              ...
              <div onClick={() => setShowCustomer(!showCustomer)}
                className="bg-slate-50 border border-slate-200 dark:border-slate-700 p-4 rounded-2xl flex items-center cursor-pointer hover:border-sky-300 hover:bg-sky-50 transition-colors mb-4">
                <div className="w-10 h-10 bg-white rounded-xl border border-slate-200 dark:border-slate-700 flex items-center justify-center mr-3">
                  <UserPlus className="w-5 h-5 text-slate-400" />
                </div>
                <div>
                  <p className="text-sm font-black text-slate-800 dark:text-slate-100">{customerName || 'Walk-in Customer'}</p>
              ...
              {showCustomer && (
                <input value={customerName} onChange={e => setCustomerName(e.target.value)}
                  ...
                  className="w-full border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 mb-4" />
              )}
            </div>
