...
import { Search, LayoutDashboard, History, Download, ChevronDown,
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans">
      <aside className="w-64 bg-slate-900 flex flex-col fixed h-full z-20">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          ...
          <Link to="/movements" className="flex items-center px-4 py-3 rounded-xl text-sm font-medium bg-slate-800 text-white relative overflow-hidden mt-1">
            <div className="absolute left-0 top-0 bottom-0 w-1 bg-sky-500 rounded-l-xl" />
            <History className="w-5 h-5 mr-3" /> Stock Movements
      ...
      <main className="flex-1 ml-64 flex flex-col min-h-screen">
        <header className="h-20 bg-white border-b border-slate-200 px-8 flex items-center justify-between sticky top-0 z-10">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Stock Movement History</h1>
            <p className="text-sm text-slate-500">Audit Trail & Logistics Log · {total} records</p>
          </div>
          <button onClick={exportCSV} className="bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors flex items-center shadow-sm">
        ...
        <div className="p-8 flex-1 space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[160px]">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">From Date</label>
            ...
            <div className="flex-1 min-w-[160px]">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">To Date</label>
              <div className="relative">
                <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            ...
            <div className="flex-1 min-w-[160px]">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Movement Type</label>
              <div className="relative">
            ...
            <div className="flex-1 min-w-[160px]">
              ...
              <div className="relative">
                ...
                <select value={branchId} onChange={e => { setBranchId(e.target.value); setPage(1); }}
                  className="w-full pl-10 pr-10 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm font-medium outline-none focus:ring-2 focus:ring-sky-500 appearance-none cursor-pointer">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                {loading ? (
                  ...
                  return (
                    <tr key={m.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-5 font-semibold text-slate-700">{new Date(m.created_at).toLocaleString('en-RW', { dateStyle: 'medium', timeStyle: 'short' })}</td>
                      <td className="px-6 py-5 font-bold text-sky-500">#{m.id}</td>
                      <td className="px-6 py-5">
                        <p className="font-bold text-slate-800">{m.product_name || '—'}</p>
                      ...
                      <td className="px-6 py-5 text-slate-500 font-medium">{m.user_name || 'System'}</td>
                    </tr>
                  );
                })}
              </tbody>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between text-sm">
              <span className="text-slate-500 font-medium">Showing {Math.min((page-1)*LIMIT+1, total)}–{Math.min(page*LIMIT, total)} of {total} movements</span>
