  ...
  return (
    <div className="flex flex-col min-h-screen dark:bg-slate-950">
      <main className="flex-1 flex flex-col min-h-screen dark:bg-slate-950">
        <header className="h-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-8 flex items-center sticky top-0 z-10">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Bulk Import</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">Add or update products via CSV</p>
          </div>
        </header>
        ...
        <div className="p-8 space-y-6 flex-1 max-w-4xl">
          ...
          {/* Step 1 */}
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-7 flex items-center justify-between">
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-1">Prepare your data</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400">Download the CSV template with the correct column format.</p>
              </div>
            </div>
            <button onClick={downloadTemplate} className="flex items-center px-5 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shrink-0">
              <FileSpreadsheet className="w-4 h-4 mr-2 text-slate-400" /> Download Template
            </button>
          </div>

          {/* Step 2 */}
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-7">
            <div className="flex items-center mb-6">
              <div className="w-10 h-10 rounded-full bg-sky-50 flex items-center justify-center mr-5 shrink-0 border border-sky-100">
                <span className="text-sky-500 font-bold">2</span>
              </div>
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Upload File</h3>
            </div>
            <div
          ...
          {preview.length > 0 && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-7">
              <div className="flex items-start mb-6">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-1">Data Preview & Validation</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400">Showing first 5 rows. Rows with errors will be skipped.</p>
                </div>
              </div>

              <div className="bg-slate-50 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden mb-6">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-100 dark:bg-slate-800 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider border-b border-slate-200 dark:border-slate-700">
                  ...
                  <tbody className="text-sm divide-y divide-slate-200">
                    {preview.map((row, i) => (
                      <tr key={i} className={row._valid ? 'bg-white' : 'bg-rose-50/30'}>
                        <td className="px-5 py-3 text-slate-500 dark:text-slate-400 font-semibold">{row._rowNum}</td>
                        <td className="px-5 py-3 font-mono font-bold text-slate-700 dark:text-slate-300">{row.sku || <span className="text-rose-500">MISSING</span>}</td>
                        <td className="px-5 py-3 font-semibold text-slate-800 dark:text-slate-100">{row.name || '—'}</td>
                        <td className="px-5 py-3 text-slate-600 dark:text-slate-400">{row.price || '—'}</td>
              ...
              <div className="flex justify-end gap-3">
                <button onClick={() => { setFile(null); setPreview([]); setResult(null); }}
                  className="px-6 py-2.5 bg-white border border-slate-300 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  Cancel
                </button>
