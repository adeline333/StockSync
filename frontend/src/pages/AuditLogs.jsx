    ...
    <div className="flex flex-col min-h-screen">
      <main className="flex-1 flex flex-col min-h-screen">
        ...
        <header className="h-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-8 flex items-center justify-between sticky top-0 z-10">
          <div>
            <h1 className="text-2xl font-black text-slate-800 dark:text-slate-100">System Audit Trail</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">Real-time user activity and security events</p>
          </div>
          <button onClick={exportCSV} className="flex items-center gap-2 bg-white dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700 border border-slate-200 text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm transition">
        ...
        <div className="p-8 flex-1 space-y-6">
          {/* Filters */}
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-wrap gap-4">
            <div className="flex-1 min-w-[140px]">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest block mb-1.5">Date Range</label>
              <select value={range} onChange={e => setRange(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700 border border-slate-200 text-sm font-semibold text-slate-700 py-2.5 px-3 rounded-lg outline-none focus:ring-2 focus:ring-amber-400">
            ...
            <div className="flex-1 min-w-[140px]">
              ...
              <select value={userId} onChange={e => setUserId(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700 border border-slate-200 text-sm font-semibold text-slate-700 py-2.5 px-3 rounded-lg outline-none focus:ring-2 focus:ring-amber-400">
            ...
            <div className="flex-1 min-w-[160px]">
              ...
              <select value={action} onChange={e => setAction(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700 border border-slate-200 text-sm font-semibold text-slate-700 py-2.5 px-3 rounded-lg outline-none focus:ring-2 focus:ring-amber-400">
            ...
            <div className="flex-[2] min-w-[200px]">
              ...
              <form onSubmit={handleSearch} className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder="Search description..." className="w-full pl-9 pr-4 py-2.5 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-amber-400" />
                </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
            <div className="grid grid-cols-[1.5fr_1fr_1.5fr_3fr_1fr] px-6 py-4 bg-slate-50 dark:bg-slate-800 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-700">
            ...
            {loading ? (
              ...
              <div className="divide-y divide-slate-50 dark:divide-slate-800">
                {logs.map(log => {
                  ...
                  return (
                    <div key={log.id} className={`grid grid-cols-[1.5fr_1fr_1.5fr_3fr_1fr] items-center px-6 py-4 border-l-4 ${log.action === 'ACCOUNT_LOCKED' ? 'border-l-rose-500 bg-rose-50/30 dark:bg-rose-950/20' : log.action.includes('PASSWORD') ? 'border-l-amber-400 bg-amber-50/20 dark:bg-amber-950/20' : 'border-l-transparent'} hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors`}>
                      <div>
                        <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{new Date(log.created_at).toLocaleString('en-RW', { dateStyle: 'medium', timeStyle: 'short' })}</p>
                        <p className="text-xs text-slate-400 mt-0.5">CAT (UTC+2)</p>
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{log.user_name || 'System'}</p>
                      ...
                      <div>
                        <span className="text-xs font-mono bg-slate-100 dark:bg-slate-800 text-slate-500 px-2 py-1 rounded-lg">{log.ip_address || '—'}</span>
                      </div>
                    </div>
                  );
                })}
