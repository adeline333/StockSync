    ...
    <div className="flex flex-col min-h-screen dark:bg-slate-950">
      <main className="flex-1 flex flex-col min-h-screen dark:bg-slate-950">
        <header className="h-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-8 flex items-center justify-between sticky top-0 z-10">
          <h1 className="text-2xl font-black text-slate-800 dark:text-slate-100">Sales History</h1>
          <form onSubmit={e => { e.preventDefault(); setSearch(searchInput); setPage(1); }} className="relative w-80">
            ...
            <input value={searchInput} onChange={e => setSearchInput(e.target.value)}
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 dark:border-slate-700 rounded-full text-sm outline-none focus:ring-2 focus:ring-sky-500" />
          </form>
        </header>
        ...
        <div className="flex-1 p-8 overflow-y-auto bg-slate-50 dark:bg-slate-950 flex gap-6 items-start">
          {/* Left: Order List */}
          <div className="flex-1 bg-white rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 min-h-[600px] flex flex-col">
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              ...
              <div className="relative">
                <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(1); }}
                  className="appearance-none bg-slate-50 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 text-sm font-semibold py-1.5 pl-3 pr-8 rounded-lg outline-none focus:ring-2 focus:ring-sky-500 cursor-pointer">
            ...
            {totalPages > 1 && (
              <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-sm">
                <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page===1}
                  className="px-3 py-1.5 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 dark:text-slate-400 hover:bg-slate-50 disabled:opacity-40">&lt;</button>
                <span className="text-slate-500 dark:text-slate-400 font-medium">Page {page} of {totalPages}</span>
          ...
          {selected ? (
            <div className="flex-[1.5] flex gap-5">
              {/* Receipt */}
              <div className="bg-white shadow-[0_10px_30px_-10px_rgba(0,0,0,0.15)] flex-1 min-h-[600px] border border-slate-200 dark:border-slate-700 rounded-sm relative overflow-hidden">
                <div ref={printRef} className="p-8 font-mono">
                  <div className="text-center mb-6 border-b border-dashed border-slate-300 pb-6">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">Kigali City, Rwanda<br/>TIN: 123-456-789<br/>Tel: +250 788 123 456</p>
                  </div>
                  ...
                  <div className="flex justify-between text-xs text-slate-600 dark:text-slate-400 mb-6 pb-4 border-b border-dashed border-slate-300">
                  ...
                  <div className="space-y-3 mb-6 border-b border-dashed border-slate-300 pb-6">
                    {selectedItems.map(item => (
                      <div key={item.id} className="text-sm">
                        ...
                        <div className="flex justify-between text-slate-500 dark:text-slate-400 text-xs font-semibold">
                  ...
                  <div className="space-y-1 mb-6 pb-6 border-b border-dashed border-slate-300">
                    <div className="flex justify-between text-sm text-slate-600 dark:text-slate-400 font-bold">
                    ...
                    <div className="flex justify-between text-base text-slate-900 font-black mt-2 pt-2 border-t border-slate-200 dark:border-slate-700">
              ...
              <div className="w-48 shrink-0 flex flex-col gap-3">
                ...
                <button className="w-full bg-white hover:bg-slate-50 text-slate-600 dark:text-slate-400 font-bold py-4 rounded-xl flex items-center justify-center border border-slate-200 dark:border-slate-700 shadow-sm transition-colors">
                  <Mail className="w-4 h-4 mr-2" /> Email
                </button>
                {selected.status === 'completed' && (
                  <div className="border-t border-slate-200 dark:border-slate-700 mt-2 pt-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md mx-4 p-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Void Order</h2>
              <button onClick={() => setVoidModal(false)} className="text-slate-400 hover:text-slate-600 dark:text-slate-400"><X className="w-5 h-5" /></button>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">This will void <strong>{selected?.order_number}</strong> and restore inventory. This cannot be undone.</p>
            <div className="mb-4">
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Reason (optional)</label>
            ...
            <div className="flex gap-3">
              <button onClick={() => setVoidModal(false)} className="flex-1 py-3 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-50">Cancel</button>
