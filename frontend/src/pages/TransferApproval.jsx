...
import { LayoutDashboard, ArrowLeftRight, ListChecks, AlertCircle,
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans">
      <aside className="w-64 bg-slate-900 flex flex-col fixed h-full z-20">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          ...
          <Link to="/transfers/approvals" className="flex items-center px-4 py-3 rounded-xl text-sm font-medium bg-slate-800 text-white relative overflow-hidden mt-1">
            ...
            <ListChecks className="w-5 h-5 mr-3" /> Approvals
      ...
      <main className="flex-1 ml-64 flex flex-col min-h-screen">
        <header className="h-20 bg-white border-b border-slate-200 px-8 flex items-center justify-between sticky top-0 z-10">
          <div>
            <h1 className="text-2xl font-black text-slate-800">Transfer Approvals</h1>
            <p className="text-sm text-slate-500">Review and authorize stock movements</p>
          </div>
          <div className="flex bg-slate-50 border border-slate-200 rounded-lg p-1.5">
            {[
              ...
              <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSelected(null); setSelectedItems([]); }}
                className={`px-5 py-2 rounded-md text-sm font-bold transition-all ${activeTab === tab.id ? 'bg-white text-sky-600 shadow-sm border border-slate-100' : 'text-slate-500 hover:text-slate-800'}`}>
                {tab.label}
              </button>
            ))}
          </div>
        ...
        <div className="p-8 flex-1 flex gap-6 overflow-hidden">
          ...
          <div className="w-80 flex flex-col gap-3 overflow-y-auto shrink-0 pb-4">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">Request Queue</p>
            {loading ? (
              ...
              <div key={t.id} onClick={() => selectTransfer(t)}
                ...
                <div className="flex justify-between items-start mb-3">
                  <h3 className="text-sm font-black text-slate-800">{t.transfer_number}</h3>
                  {t.priority === 'high' && (
                ...
                <div className="flex items-center justify-between mt-2">
                  <span className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-full border ${statusStyle[t.status] || 'bg-slate-50 text-slate-500 border-slate-200'}`}>
                    {t.status}
                  </span>
                </div>
              </div>
            ...
            {!selected ? (
              ...
              <>
                <div className="flex justify-between items-start p-8 border-b border-slate-100">
                  <div>
                    <h2 className="text-xl font-black text-slate-800 mb-1">{selected.transfer_number}</h2>
                    <p className="text-sm text-slate-500">{new Date(selected.created_at).toLocaleString('en-RW', { dateStyle: 'medium', timeStyle: 'short' })}</p>
                  </div>
                  <span className={`text-xs font-black uppercase px-4 py-2 rounded-full border ${statusStyle[selected.status] || 'bg-slate-50 text-slate-500 border-slate-200'}`}>
                    {selected.status}
                  </span>
                </div>
                ...
                {/* Notes */}
                {selected.reason && (
                  <div className="mx-8 my-4 bg-slate-50 border border-slate-200 rounded-xl p-4 relative overflow-hidden">
                    <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-slate-300 rounded-l-xl"/>
                    <p className="text-xs font-bold text-slate-500 mb-1">Transfer Reason:</p>
                    <p className="text-sm font-semibold text-slate-700 italic">"{selected.reason}"</p>
                  </div>
                )}

                {/* Items */}
                <div className="flex-1 overflow-y-auto px-8 py-4">
                  <h3 className="text-sm font-black text-slate-800 mb-3">Items to Transfer</h3>
                  <div className="border border-slate-200 rounded-xl overflow-hidden">
                    <div className="grid grid-cols-[3fr_1.5fr_1fr] bg-slate-50 px-5 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200">
