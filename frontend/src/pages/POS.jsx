  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden">
      ...
      <main className="flex-1 ml-[100px] flex flex-col h-screen overflow-hidden mr-[420px]">
        <header className="h-20 bg-white border-b border-slate-200 px-6 flex items-center shrink-0">
          <div className="relative flex-1 max-w-2xl bg-slate-50 border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-sky-500 transition-all">
        ...
        <div className="px-6 py-4 border-b border-slate-200 bg-white shrink-0 overflow-x-auto">
          <div className="flex space-x-2">
            {categories.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)}
                className={`px-5 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all ${activeCategory === cat ? 'bg-slate-900 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>
                {cat}
              </button>
            ))}
          </div>
              ...
              {products.map(prod => {
                ...
                return (
                  <div key={prod.id} onClick={() => addToCart(prod)}
                    ...
                    <div className="w-full aspect-square bg-slate-50 rounded-xl mb-3 flex items-center justify-center border border-slate-100 relative">
                      <Package className="w-10 h-10 text-slate-300" />
                      <div className="absolute top-2 left-2 bg-white px-1.5 py-0.5 rounded text-[9px] font-black text-slate-500 uppercase tracking-widest shadow-sm">{prod.category || '—'}</div>
                    ...
                    <h4 className="font-bold text-slate-800 text-sm leading-tight mb-1 line-clamp-2">{prod.name}</h4>
      ...
      <aside className="w-[420px] bg-white border-l border-slate-200 flex flex-col h-screen fixed right-0 z-20">
            ...
            <div key={item.id} className="px-6 py-4 border-b border-slate-50 group">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex items-center gap-2">
                  <p className="text-base font-black text-slate-800">{(item.price * item.qty).toLocaleString()}</p>
              ...
              <div className="flex items-center bg-slate-100 rounded-lg p-1 w-fit border border-slate-200">
                <button onClick={() => updateQty(item.id, -1)} className="w-8 h-7 flex items-center justify-center text-slate-500 hover:text-sky-500 hover:bg-white rounded transition-all">
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <input type="number" min="0" value={discount} onChange={e => setDiscount(e.target.value)}
                  className="w-24 text-right border border-slate-200 rounded-lg px-2 py-1 text-sm font-bold focus:outline-none focus:ring-1 focus:ring-sky-500" />
              </div>
            </div>
            <div className="flex justify-between text-sm border-t border-dashed border-slate-200 pt-3">
              <span className="text-slate-500 font-semibold">VAT (18%)</span>
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <button className="py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold text-sm hover:bg-slate-50 transition-colors flex items-center justify-center gap-2">
